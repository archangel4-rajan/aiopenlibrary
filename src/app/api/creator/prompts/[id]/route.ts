...
export async function PUT(
  ...
  try {
    ...
    if (error) {
      console.error("PUT /api/creator/prompts/[id] error:", { userId: user.id, promptId: id, error });
      return NextResponse.json({ error: "Internal server error" }, { status: 500 });
    }

    return NextResponse.json(data);
  } catch (err) {
    console.error("PUT /api/creator/prompts/[id] error:", err);
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error("DELETE /api/creator/prompts/[id] error:", { userId: user.id, promptId: id, error });
      return NextResponse.json({ error: "Internal server error" }, { status: 500 });
    }

    return NextResponse.json({ deleted: true });
  } catch (err) {
    console.error("DELETE /api/creator/prompts/[id] error:", err);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
